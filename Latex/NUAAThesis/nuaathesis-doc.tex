...
\end{itemize}
\begin{latex}
    \documentclass[output=electronic]{nuaathesis}
\end{latex}
...
\end{itemize}
\begin{latex}
    \documentclass[section-style=arabic]{nuaathesis}
\end{latex}
...
其中的各项信息使用 \cs{nuaasetup} 命令的方式填写，如：
\begin{latex}
    \nuaasetup{
        title  = {论文中文题目},
...
有几点说明：
\begin{itemize}
    \item \cs{nuaasetup} 使用 \pkg{kvsetkeys} 机制，配置项之间不能有空行，否则会报错。
...
\DescribeOption{keywords*}
摘要的关键词应使用 \cs{nuaasetup} 的接口进行设置，
只要在摘要结束前即可，比如：
\begin{latex}
    \begin{abstract}
        这里是摘要。
        \nuaasetup{keywords = {论文；摘要；关键词}}
    \end{abstract}
\end{latex}
